...
<br>
<h3>this is my custom create account</h3>
<br>
<h3>${name}</h3>
...
<liferay-portlet:resourceURL id="/login/create_account" var="cancelOTPActionURL"  >
<portlet:param name="cancelOTPAction" value="cancelOTP" />
</liferay-portlet:resourceURL>

<liferay-portlet:resourceURL id="/login/create_account" var="createOTPActionURL" copyCurrentRenderParameters="false" >
<portlet:param name="createOTPAction" value="createOTP" />
</liferay-portlet:resourceURL>


<portlet:actionURL name="/login/create_account" secure="<%= PropsValues.COMPANY_SECURITY_AUTH_REQUIRES_HTTPS || request.isSecure() %>" 
var="createAccountURL" windowState="<%= LiferayWindowState.MAXIMIZED.toString() %>">
	<portlet:param name="mvcRenderCommandName" value="/login/create_account" />
</portlet:actionURL>

<aui:form action="<%= createAccountURL %>" method="post" name="fm">

<aui:input type="" id="cancelOTPRequest" name="cancelOTPRequest" value="<%= cancelOTPActionURL %>" />
<aui:input type="" id="createOTPRequest" name="createOTPRequest" value="<%= createOTPActionURL %>" />
	...
	<aui:button-row>
		...
		<aui:button-row>
		<aui:button type="button" value="cancelOTPRequest"  title="CancelOtpRequest" onClick="CancelOtpRequest()" />
		</aui:button-row>
		
			<aui:button-row>
		<aui:button type="button" value="createOTPRequest"  title="createOtpRequest" onClick="createOtpRequest()" />
		</aui:button-row>
</aui:form>
<aui:script>
function CancelOtpRequest(){
alert("inisde CancelOtpRequest ****");
	    	var url1 = $("#_com_liferay_login_web_portlet_LoginPortlet_cancelOTPRequest").val();
	    	alert("CancelOtpRequest URL *** "+ url1);
    	$.ajax({
   		 dataType:"json",
   		 type:"post",
   		 url:url1,
...
function createOtpRequest(){
	alert("inisde createOtpRequest ****");
		    	var url1 = $("#_com_liferay_login_web_portlet_LoginPortlet_createOTPRequest").val();
		    	alert("createOtpRequest URL *** "+ url1);
	    	$.ajax({
	   		 dataType:"json",
	   		 type:"post",
	   		 url:url1,
